...
async function predict() {
  try {
    ...
    if (prediction[currentQuizIndex].probability.toFixed(2) >= "0.40") {
      score += 100;
      flag = true;
      setTimeout(() => {
        setQuiz();
...
function setQuiz() {
  let random = Math.floor(Math.random() * poze.length - 1);
  duplicateNot = random;
  while (duplicateNot == random) {
    random = Math.floor(Math.random() * poze.length - 1);
    if (random == duplicateNot) {
      break;
    }
  }
  if (random == duplicateNot) {
